...
from einsum import dm2rho01, dm2eT2, dm2eJ, dm2eK, dm2rho01_sep, dm2eT, cal_dipole, dm2J
...
def gen_train_data_E(path_list, a=0.9, n_samples=6, priority=0, basis='aug-cc-pvtz', grid_level=3, base_method='pbe', target_method='ccsd', e_T2=True, train=True, complex_geo=[], model=None):
    ...
    for fn in path_list:
        ...
        if 'E_pbe' not in data:
            ...
            ks.kernel()
            data['E_pbe'] = ks.e_tot
            g = guks(ks) if data['spin'] else gks(ks)
            data['F_pbe'] = -g.kernel()
            data['dm_pbe'] = ks.make_rdm1()
            data['rho01_pbe'] = dm2rho01_sep(data['dm_pbe'], data['phi01'])
            if len(data['rho01_pbe']) == 2:
        ...
        if model is not None:
            ...
            res = model.scf(data, data['basis'], grid_level=grid_level, xc='pbe', xctype='GGA')
            data.update(res)
        ...
        if 'T' in data:
            ...
            valid_list.append({'x':x, 'rho_base':data[f'rho_{base_method}'], 'gc':data['gc'], 'gw':data['gw'], 'rho_target':data.get(f'rho_{target_method}', data['rho_ccsd']),
                            ...
                            'J': F0 - (data['T'] + data['V_ext']), 'E_N': data['E_N'], 'H1':data['T'] + data['V_ext'], 'itg_2e':itg_2e,
            ...
            valid_list.append({'x':x, 'rho_base':data[f'rho_{base_method}'], 'gc':data['gc'], 'gw':data['gw'], 'rho_target':data.get(f'rho_{target_method}', data['rho_ccsd']),
                            ...
                            'E_N':data['E_N'], 'dm_target':data.get(f'dm_{target_method}', data['dm_ccsd']), 'dm_base': data[f'dm_{base_method}'],
                            'F_base': data[f'F_{base_method}'], 'F_target': data.get(f'F_{target_method}', data['F_ccsd']),
                            'phi01': data['phi01'], 'rho01': data.get(f'rho01_{target_method}', data['rho01_ccsd']),
                            })
...
if __name__ == "__main__":
    ...
    traindata =None
    model_path = os.path.join(yml.homepath, f'model/{md5value:.10}_{yml.basis}_{yml.base_method}_to_{yml.target_method}_{yml.n_samples}_newparam_zigzag.pt')
    if os.path.exists(model_path) and not yml.retrain:
        print(model_path, 'already exists.')
    else:
        traindata = gen_train_data_E(trainset, a=yml.edge_length, n_samples=yml.n_samples+1, priority=yml.train.bond_priority, basis=yml.basis, grid_level=yml.grid_level, base_method='pbe', target_method=yml.target_method, e_T2=yml.e_T2, train=True, complex_geo=yml.train.complex_geo)
        validdata = gen_train_data_E(validset, a=yml.edge_length, n_samples=yml.n_samples+1, priority=yml.train.bond_priority, basis=yml.basis, grid_level=yml.grid_level, base_method='pbe', target_method=yml.target_method, e_T2=yml.e_T2, train=False, complex_geo=yml.train.complex_geo)
        with open(os.path.join(yml.homepath, f'model/meta'), 'a') as f:
            f.write(f"{md5value:.10} ->{str(trainset)}\n")
        print(len(traindata))
        print(len(validdata))
        
        model = ModelE_pbe(device=yml.device)
        # model.load('/home/alfred/tree_regression/model/95f80d4876_aug-cc-pvdz_b3lypg_to_ccsdt_1_newparam_zigzag_old.pt')
        model.fit(traindata, validdata, model_path, lr=yml.train.lr, max_iter=yml.train.max_iter, batch_size=yml.train.batch_size, off=False)
        ...
        for iteration in range(10):
            ...
            # TODO 
            # model = ModelE_pbe(device=yml.device)
            model.fit(traindata, validdata, model_path.replace('zigzag', 'zigzag_'+str(iteration)), lr=yml.train.lr, max_iter=yml.train.max_iter, batch_size=yml.train.batch_size)
            if iteration<9:
