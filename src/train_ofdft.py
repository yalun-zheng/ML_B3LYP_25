...
def gen_train_data_e(path_list, eps=1e-7, a=0.9, n_samples=6, priority=0, basis='aug-cc-pvtz', grid_level=3, base_method='b3lyp', target_method='ccsd', e_T2=True, train=True, complex_geo=[]):
    ...
    for fn in path_list:
        if not os.path.exists(fn):
            ...
            stretch_mol(fn, priority, basis, grid_level, test=True if molname in complex_geo else False)
    # exit(0)
...
def gen_train_data_E(path_list, eps=1e-7, a=0.9, n_samples=6, priority=0, basis='aug-cc-pvtz', grid_level=3, base_method='b3lyp', target_method='ccsd', e_T2=True, train=True, complex_geo=[]):
    ...
    for fn in path_list:
        ...
        ni = dft.numint.NumInt()
        x = get_feature(data, mol, ni, a, n_samples, dm=base_method)
        valid_list.append({'x':x, 'rho_base':data[f'rho_{base_method}'], 'gc':data['gc'], 'gw':data['gw'], 'rho_target':data[f'rho_{target_method}'],
                        'fn':fn, 'atoms_charges':data['atoms_charges'], 'atoms_coords':data['atoms_coords'], 'dipole_base':data[f'dipole_{base_method}'],
                        'dipole_target':data[f'dipole_{target_method}'], 'E_base':data[f'E_{base_method}'], 'E_target':data[f'E_{target_method}'], 'I_base':data[f'I_{base_method}'],
                        'spin':data['spin'], 'charge':data['charge'],
                        'E_N':data['E_N']})
...
if __name__ == "__main__":
    ...
    from model import ModelE, ModelD
        ...
        for bond_length in bond_lengths:
            if type(bond_length)==list:
                for bd in np.linspace(*bond_length):
                    if molname == '*':
                        for mn in os.listdir(os.path.join(yml.homepath, f'data/xyz')):
                            if mn.endswith('.xyz'):
                                trainset.append(f'{mn[:-4]}_{bd:.4f}_{yml.train.bond_priority}_{yml.basis}_train_valid.pkl')
                    else:
                ...
                if molname == '*':
                    for mn in os.listdir(os.path.join(yml.homepath, f'data/xyz')):
                        if mn.endswith('.xyz'):
                            trainset.append(f'{mn[:-4]}_{bond_length:.4f}_{yml.train.bond_priority}_{yml.basis}_train_valid.pkl')
                else:
            ...
            if type(bond_length)==list:
                for bd in np.linspace(*bond_length):
                    if molname == '*':
                        for mn in os.listdir(os.path.join(yml.homepath, f'data/xyz')):
                            if mn.endswith('.xyz'):
                                validset.append(f'{mn[:-4]}_{bd:.4f}_{yml.train.bond_priority}_{yml.basis}_train_valid.pkl')
                    else:
                ...
                if molname == '*':
                    for mn in os.listdir(os.path.join(yml.homepath, f'data/xyz')):
                        if mn.endswith('.xyz'):
                            validset.append(f'{mn[:-4]}_{bond_length:.4f}_{yml.train.bond_priority}_{yml.basis}_train_valid.pkl')
                else:
    ...
    traindata =None
    model_path = os.path.join(yml.homepath, f'model/{md5value:.10}_{yml.base_method}_to_{yml.target_method}_{yml.n_samples}_e.pt')
    if os.path.exists(model_path) and not yml.retrain:
        ...
        validdata = gen_train_data_E(validset, a=yml.edge_length, n_samples=yml.n_samples+1, priority=yml.train.bond_priority, basis=yml.basis, grid_level=yml.grid_level, base_method=yml.base_method, target_method=yml.target_method, e_T2=yml.e_T2, train=False)
    ...
    model_path = os.path.join(yml.homepath, f'model/{md5value:.10}_{yml.base_method}_to_{yml.target_method}_{yml.n_samples}_d.pt')
    if os.path.exists(model_path) and not yml.retrain:
        print(model_path, 'already exists.')
    else:
        if not traindata:
        ...
        model.fit(traindata, validdata, model_path, lr=yml.train.lr*100, max_iter=yml.train.max_iter, batch_size=yml.train.batch_size)
