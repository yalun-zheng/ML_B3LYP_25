...
from train_fcn_zigzag import cal_mu, get_feature, gen_train_data_E
import time
import os
import torch
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(columns=['Molecule', 'Bond length', f'E {yml.base_method}', 'E model', 'E scf', f'E {yml.target_method}', f'Error E {yml.base_method} in kcal/mol', f'Error E model in kcal/mol', 'Error E scf in kcal/mol',
    ...
    for count, fn in enumerate(sorted(validset)):
        ...
        if count==0:
            model_e = ModelE_zigzag(device=yml.device)
        ...
        res = model_e.scf(valid, yml.basis, grid_level=3, hyb=0.2)
        
        error_F_base = np.sum((valid['F_base'] - valid['F_target'])**2)**.5
        error_F_model = np.sum((res['F_scf'] - valid['F_target'])**2)**.5
        df.loc[count] = [*os.path.basename(fn).split('_')[:2], valid['E_base'], valid['E_base']+np.sum(inference_e*valid['rho_base']*valid['gw']), res['E_scf'], valid['E_target'], (valid['E_base']-valid['E_target'])*627, (valid['E_base']+np.sum(inference_e*valid['rho_base']*valid['gw'])-valid['E_target'])*627, (res['E_scf']-valid['E_target'])*627,
