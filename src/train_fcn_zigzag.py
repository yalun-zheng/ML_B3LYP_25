...
def get_feature1(data, mol, ni, a, dm='b3'):
    ...
    # return np.vstack((rho01[0], np.zeros(len(tau[0])), tau[0])), np.vstack((rho01[1], np.zeros(len(tau[1])), tau[1]))
...
def gen_train_data_E(path_list, a=0.9, n_samples=6, priority=0, basis='aug-cc-pvtz', grid_level=3, base_method='b3lyp', target_method='ccsd', e_T2=True, train=True, complex_geo=[], model=None):
    ...
    for fn in path_list:
        ...
        if model is not None:
            ...
            res = model.scf(data, data['basis'], grid_level=grid_level)
            data.update(res)
...
if __name__ == "__main__":
    ...
    from model import ModelE_zigzag
    ...
    if os.path.exists(model_path) and not yml.retrain:
        ...
        model = ModelE_zigzag(device=yml.device)
